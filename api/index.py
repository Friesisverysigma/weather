...
import os
import traceback
from zip_database import get_zip_codes_for_states, generate_sample_zip_codes_for_state
...
class WeatherFetcher:
    ...
    def fetch_all_weather_data(self):
        ...
        # Use ThreadPoolExecutor for concurrent requests with smaller batch size for large datasets
        max_workers = min(5, max(1, total_zips // 1000))  # Adjust workers based on dataset size
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_zip):
                ...
                try:
                    ...
                    # Send progress update every 10 ZIP codes or at the end
                    if completed % 10 == 0 or completed == total_zips:
                        self.progress_queue.put({
                            'type': 'progress',
                            'current': completed,
                            'total': total_zips,
                ...
                time.sleep(0.05)  # Reduced delay for large datasets
...
def fetch_weather():
    ...
    try:
        ...
        # Handle state-based ZIP code fetching
        if states and not zip_codes:
            print(f"Fetching ZIP codes for states: {states}")
            all_zip_codes = []
            
            # For demonstration, we'll generate sample ZIP codes
            # In a real implementation, you would query a comprehensive database
            for state in states:
                state_zips = generate_sample_zip_codes_for_state(state, 50)  # Limit for demo
                all_zip_codes.extend(state_zips)
                print(f"Added {len(state_zips)} ZIP codes for {state}")
            
            zip_codes = all_zip_codes
            print(f"Total ZIP codes to process: {len(zip_codes)}")
        
        if not zip_codes:
            return jsonify({'error': 'ZIP codes or states are required'}), 400
